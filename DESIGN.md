...
## Touch Button Sizing（スマホ用ボタンサイズ・間隔）
スマホ（≤480px）でのタップ操作性を担保するため、ボタンは**優先度**に応じてサイズと間隔を定める。ここで定めるサイズ・間隔は**スマホ幅（≤480px）にのみ適用**する。PC では従来のボタン高さ（Buttons セクション参照）を用いる。

### ボタンサイズ（高さ／最小タップ領域）
優先度は「そのアクションがユーザーの主目的にどれだけ近いか」で判断する。

| 優先度 | 変数                 | 高さ | 用途の例                             |
|--------|----------------------|------|--------------------------------------|
| 高     | `--btn-touch-high`   | 72px | 画面の主目的アクション（送信・実行等）|
| 中     | `--btn-touch-mid`    | 60px | サブアクション（絞り込み・切替等）    |
| 低     | `--btn-touch-low`    | 44px | 補助アクション（軽い操作・アイコン等）|

> 全値とも 4 の倍数（72=4×18 / 60=4×15 / 44=4×11）で Spacing ルールに準拠。
> 低優先度は Touch セクションの最小タップ領域（44×44px）と一致させている。

### ボタン間隔（隣接するボタン同士のギャップ）
**小さいボタンほど誤タップしやすいため、間隔を広く取る。**（＝サイズと間隔は反比例）

| 対象ボタン     | 変数              | 間隔      | 備考                        |
|----------------|-------------------|-----------|-----------------------------|
| 大（72px）     | `--gap-btn-large` | 12〜24px  | 面積が大きく誤タップしにくい |
| 中（60px）     | `--gap-btn-mid`   | 24〜36px  | 中間                        |
| 小（44px）     | `--gap-btn-small` | 36〜48px  | 誤タップ防止で最も広く取る  |
| テキストボタン | `--gap-btn-text`  | 12px      | 文字リンク的な軽い要素      |

> いずれも下限値をデフォルトとし、要素が密集する場面では上限まで広げてよい。
> 間隔値はすべて 4 の倍数のため、Spacing ルールと矛盾しない。

### CSS変数の定義
```css
:root {
  /* スマホ用ボタン高さ */
  --btn-touch-high: 72px;   /* 高優先度 */
  --btn-touch-mid:  60px;   /* 中優先度 */
  --btn-touch-low:  44px;   /* 低優先度（最小タップ領域 44px と統一） */

  /* スマホ用ボタン間隔（下限をデフォルトに、密集時のみ上限まで） */
  --gap-btn-large: 12px;    /* 大ボタン: 12〜24px */
  --gap-btn-mid:   24px;    /* 中ボタン: 24〜36px */
  --gap-btn-small: 36px;    /* 小ボタン: 36〜48px */
  --gap-btn-text:  12px;    /* テキストボタン */
}
```
...
@media (max-width: 480px) {
  ...
  .btn-primary,
  .btn-danger {
    min-height: var(--btn-touch-high);
    font-size: var(--font-size-lg);   /* タップしやすさ優先で1段上げてよい */
  }

  /* 中優先度ボタン */
  .btn-secondary {
    min-height: var(--btn-touch-mid);
  }

  /* 低優先度ボタン */
  .btn-ghost,
  .icon-btn {
    min-height: var(--btn-touch-low);
    min-width:  var(--btn-touch-low);
  }

  /* ボタンが横／縦に並ぶコンテナの間隔 */
  .btn-group-large { display: flex; gap: var(--gap-btn-large); }
  .btn-group-mid   { display: flex; gap: var(--gap-btn-mid); }
  .btn-group-small { display: flex; gap: var(--gap-btn-small); }

  /* テキストボタンの並び */
  .text-btn-group  { display: flex; gap: var(--gap-btn-text); }
}
```

### Do's and Don'ts
- **Do**: スマホでは優先度に応じて 72 / 60 / 44px を使い分ける
- **Do**: 小さいボタンほど周囲の間隔を広く取り、誤タップを防ぐ
- **Do**: 間隔は下限をデフォルトにし、密集時のみ上限まで広げる
- **Don't**: 優先度の異なるボタンを同一サイズで並べない（視覚的な重要度が伝わらなくなる）
- **Don't**: 高優先度ボタンを1画面に複数置かない（Buttons セクションの原則を継承）
...
- **Do**: スマホのボタンは優先度に応じて 72 / 60 / 44px を使い分け、小さいボタンほど間隔を広く取る
