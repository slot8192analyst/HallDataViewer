...
| 項目 | 内容 |
|------|------|
| 種別 | 完全な**静的サイト**（ビルド不要・サーバー不要、ブラウザだけで動作） |
| 言語 | バニラ JavaScript (ES5寄り / モジュールバンドラなし) + HTML + CSS |
| 外部ライブラリ | [Chart.js](https://cdn.jsdelivr.net/npm/chart.js)（CDN読み込み）のみ |
| データ | `data/YYYY_MM.json`（月別の台データ）を `fetch` で読み込み |
| 状態保存 | `localStorage` + URLクエリパラメータ（タグ・フィルター・選択日など） |
