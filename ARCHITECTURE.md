...
| フィールド | 型 | 説明 |
|---|---|---|
| `機種名` | string | 機種名（唯一の文字列フィールド） |
| `台番号` | int | 台番号 |
| `G数` | int | ゲーム数 |
| `差枚` | int | 差枚数（プラスが出玉超過、マイナスが投入超過） |
| `BB` | int | ビッグボーナス回数 |
| `RB` | int | レギュラーボーナス回数 |
| `ART` | int | ART回数（非ART機は常に 0） |

> **廃止フィールド**: `合成確率`・`BB確率`・`RB確率`・`ART確率` は削除済み。
> 確率が必要な場合はフロント側で `G数 / (BB + RB)` 等により導出する。
...
ファイル名: `data/YYYY_MM.json`（例: `data/2026_08.json`）

#### 形式の仕様
- トップレベルはオブジェクト。キーは `YYYY_MM_DD`（アンダースコア区切り）
- 各キーの値は**台レコードの配列**（1日 ≒ 400台）
- **1台1行**のコンパクト出力（インデント2スペース＋各台レコードは1行に収める）
- 台レコード内のセパレータは `", "` と `": "`（スペースあり・可読性重視）

#### サンプル（実際のファイルの形式）
```json
{
  "2026_08_01": [
    {"機種名": "ネオアイムジャグラーEX", "台番号": 881, "G数": 9668, "差枚": 1267, "BB": 34, "RB": 44, "ART": 0},
    {"機種名": "ネオアイムジャグラーEX", "台番号": 882, "G数": 6406, "差枚": 1221, "BB": 27, "RB": 20, "ART": 0},
    {"機種名": "ハナハナ鳳凰",         "台番号": 501, "G数": 3941, "差枚": -942, "BB": 10, "RB": 17, "ART": 0}
  ],
  "2026_08_02": [
    ...
  ]
...
- 変換スクリプト: `converter/convert_to_new_format.py`（バックアップから再変換する場合は `data/backup/` を入力とする）
...
- 🐙=上位（タコだし）、💀=下位（死に台）。表示順位 `[1,2,3,4,5]` をカスタム可（上位5・下位5まで対応。設定UIのチェックボックスで個別に表示/非表示を切り替え可）
